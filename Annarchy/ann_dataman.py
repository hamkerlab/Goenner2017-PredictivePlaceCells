...
rotated_plot = 1
occupancy_plot = 1 # 0
i_net_occ = 0 # 1 # 0 # No bump movement for network 1 ?!
...
identifier = 'recoded_ann_dataman_noSim'+'_N'+str(N)+'_'+str(nTrials)+'trials'
...
latencies, endpoints, weights, seqcount, seqstart, seqend, rand_nav_time, \
        goal_nav_time, focal_search_time, occupancyMap, weight_array_home, weight_array_away, center_mat_plot, center_mat_array, goal_index_array = list(zip(*list(dataman.values())))
...
if learning_prec_plot:
        ...
        xind, yind = net_init.xypos(list(range(n_grid**2)), n_grid)
        ...
        trial_list = list(range(nTrials-2, nTrials, 2))
...
if speed_plot:
    ...
    nojumps = 0
    rest = 0
    for i_netw in range(n_valid_nets): # n_valid_nets
        for i_trial in range(nTrials): # 
...
if occupancy_plot:
    ...
    bincount = zeros(36)
    for j_trial in range(0, nTrials, 2): # Home trials
    ...
    nonhome_bins = list(range(0, home_index)) + list(range(home_index+1, 36))
    figure()
    #subplot(121)
    ...
    for i_trial in range(min(nTrials, 12)):
...
#    figure()
    ...
    if combined_Both_singlepics:
	    ...
	    #axis('off')
            ...
            for i_trial in range(min(nTrials, disp_trials)):
                # Weights
                placeValueMat = zeros([n_grid, n_grid])
                if mod(i_trial, 2) == 0:
                        # Home trials:
                        for iNeuron in range(len(weight_array_home[i_netw][i_trial])):
                            x,y = net_init.xypos(iNeuron, n_grid)
                            placeValueMat[x, y] = weight_array_home[i_netw][i_trial][iNeuron] * 1.0e-3 # / 1.0e-9
                        figure(figsize=(4,4), dpi=dpisize)
	                ...
	                #axis('off')
                        ...
                        # Away trials:
                        for iNeuron in range(len(weight_array_away[i_netw][i_trial])):
                            x,y = net_init.xypos(iNeuron, n_grid)
                            placeValueMat[x, y] = weight_array_away[i_netw][i_trial][iNeuron] * 1.0e-3 #  / 1.0e-9
                        figure(figsize=(4,4), dpi=dpisize)
    ...
    if combined_plot_Both:
            ...
            #wmax = 0
            wmax = weight_array_home.max() * 1.0e-3
            ...
            for i_trial in range( min(nTrials, disp_trials)-1, -1, -1): # Reversed order!
                ...
                if mod(i_trial, 2) == 0:
                    for iNeuron in range(len(weight_array_home[i_netw][i_trial])):
                        x,y = net_init.xypos(iNeuron, n_grid)
                        placeValueMat[int(x), int(y)] = weight_array_home[i_netw][i_trial][iNeuron] * 1.0e-3 #  / 1.0e-9
                    ...
                    for iNeuron in range(len(weight_array_away[i_netw][i_trial])):
                        x,y = net_init.xypos(iNeuron, n_grid)
                        placeValueMat[int(x), int(y)] = weight_array_away[i_netw][i_trial][iNeuron] * 1.0e-3 #  / 1.0e-9
    ...
    if combined_plot_Home:
            ...
            for i_trial in range( min(int(int( ceil(nTrials/2.0) )), disp_trials)-1, -1, -1): # Reversed order!
                # Bump movement
                ...
                for iNeuron in range(len(weight_array_home[i_netw][i_trial])):
                        x,y = net_init.xypos(iNeuron, n_grid)
                        placeValueMat[int(x), int(y)] = weight_array_home[i_netw][2*i_trial][iNeuron] * 1.0e-3 #  / 1.0e-9
                if placeValueMat.max() > wmax:
                    wmax = placeValueMat.max()
                subplot(3, disp_trials, i_trial + 1 + 2*disp_trials)
...
if figs_for_movie:
    ...
    wmax_home = 0
    for k_trial in range(4): # 5
            placeValueMat = zeros([n_grid, n_grid])
            for iNeuron in range(len(weight_array_home[i_netw][k_trial])):
                x,y = net_init.xypos(iNeuron, n_grid)
...
if weights_for_movie:
    ...
    for i_trial in range(n_trial, n_trial+1):
            placeValueMat = zeros([n_grid, n_grid])
            for iNeuron in range(len(weight_array_home[i_netw][i_trial])):
                x,y = net_init.xypos(iNeuron, n_grid)
                placeValueMat[x, y] = weight_array_home[i_netw][2*i_trial][iNeuron]
